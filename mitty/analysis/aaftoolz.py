...
def save_as_aaf(seq_dict, output_dir, titer):
  ...
  :param output_dir:
  :param titer:
  :return: output file name (generated by tempfile)
  ...
  with open(aaf_fname, 'w') as fp:
    for template in titer:
      for mate in template:
        ...
        fp.write(
          '{chrom}\t{pos}\t{d_err}\t{wchrom}\t{wpos}\t{mq}\t{vl}\n'.format(
            chrom=ri.chrom,
            pos=ri.pos,
            d_err=d_err,
            wchrom=seq_dict[rd.reference_id] if d_err else '',
            wpos=rd.pos if d_err else '',
            mq=rd.mapping_quality,
...
def aaf_iter(fp, contig_q):
  ...
  for contig in iter(contig_q.get, None):
    cnt = 0
    for cnt, line in enumerate(fp.fetch(contig)):
      chrom, pos, d_err, wchrom, wpos, MQ, v_list = line.split('\t')
      yield chrom, int(pos), int(d_err), wchrom, 0 if wpos == '' else int(wpos), \
            int(MQ), [int(v) for v in v_list.split(';') if v is not '']
